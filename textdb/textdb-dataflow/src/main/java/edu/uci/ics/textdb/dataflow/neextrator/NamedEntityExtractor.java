 ...
 *         <p>
 *         Wrap the Stanford NLP Named Entity Recognizer as an operator. This
 *         operator would recognize 7 classes: Location, Person, Organization,
 *         Money, Percent, Date and Time. Return the recoginized data as a list
 *         of spans that are appended to the original tuple as a field.
 *         <p>
 *         For example: Given tuple with two fields: sentence1, sentence2.
 *         tuple: ["Google is an organization.",
 *         "Its headquarter is in Mountain View."]
 *         <p>
 *         Append a list of spans then return: [
 *         "sentence1,0,6,Google, NE_ORGANIZATION",
 *         "sentence2,22,25,Mountain View, NE_LOCATION"]
 */
...
public class NamedEntityExtractor implements IOperator {
     ...
     *        Return format is a Tuple that contains only one field which is a
     *        list of spans of the result.
     * @overview First get a tuple from the source operator then process it
     *           using the Stanford NLP package. for all recognized words,
     *           compute their spans and return all as a list.
     */
    @Override
     ...
     * @about This function takes an (TextField) IField and a String (the
     *        field's name) as input and uses the Stanford NLP package to
     *        process the field. It returns a list of spans In the returning
     *        span: Value -> the word itself Key -> NE_Constant
     * @overview Using the Stanford NLP package to process the textField value.
     *           First set up a pipeline of Annotators for the string:
     *           TokenizerAnnotator, SentencesAnnotator, PartOfSpeechAnnotator,
     *           LemmaAnnotator and NamedEntityTagAnnotator. The order is
     *           mandatory because they have dependencies. After the pipeline,
     *           each token is wrapped as a CoreLabel and each sentence is
     *           wrapped as CoreMap. Each annotator adds its annotation to the
     *           CoreMap(sentence) or CoreLabel(token) object.
     *           <p>
     *           After the pipeline is done, scan each CoreLabel(token) for its
     *           NamedEntityAnnotation, if it's a valid value (not 'O'), then
     *           makes it a span and add to the return list. The Stanford NLP
     *           constants are mapped into the NE constants. The NLP package has
     *           annotations for the start and end position of a token and it
     *           perfectly matches the span design so we just use them.
     *
     */
    ...
    /**
     * This function takes a Stanford NLP Constant (The 7 Classes as
     * LOCATION,PERSON,ORGANIZATION,MONEY,PERCENT,DATE, TIME and NUMBER) and
     * returns the corresponding NE Constant.
     *
     * @param NLPConstant
     * @return
     */
