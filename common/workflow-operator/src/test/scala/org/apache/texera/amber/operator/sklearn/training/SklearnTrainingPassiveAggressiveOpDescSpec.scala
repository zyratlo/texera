...
class SklearnTrainingPassiveAggressiveOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingPassiveAggressiveOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingPassiveAggressiveOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Passive Aggressive"
    info.operatorDescription shouldBe "Sklearn Training: Passive Aggressive Operator"
  ...
  "SklearnTrainingPassiveAggressiveOpDesc" should "default its config fields" in {
    val d = new SklearnTrainingPassiveAggressiveOpDesc
  ...
  "SklearnTrainingPassiveAggressiveOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingPassiveAggressiveOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.linear_model import PassiveAggressiveClassifier")
    code should include("make_pipeline")
    code should include("Training: Passive Aggressive")
  }

  "SklearnTrainingPassiveAggressiveOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingPassiveAggressiveOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingPassiveAggressive\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingPassiveAggressiveOpDesc]
    val r = restored.asInstanceOf[SklearnTrainingPassiveAggressiveOpDesc]
