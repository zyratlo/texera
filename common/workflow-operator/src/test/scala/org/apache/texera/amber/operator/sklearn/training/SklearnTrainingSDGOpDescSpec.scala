...
class SklearnTrainingSDGOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingSDGOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingSDGOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Stochastic Gradient Descent"
    info.operatorDescription shouldBe "Sklearn Training: Stochastic Gradient Descent Operator"
  ...
  "SklearnTrainingSDGOpDesc" should "default its config fields" in {
    val d = new SklearnTrainingSDGOpDesc
  ...
  "SklearnTrainingSDGOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingSDGOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.linear_model import SGDClassifier")
    code should include("make_pipeline")
    code should include("Training: Stochastic Gradient Descent")
  }

  "SklearnTrainingSDGOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingSDGOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingSDG\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingSDGOpDesc]
    val r = restored.asInstanceOf[SklearnTrainingSDGOpDesc]
