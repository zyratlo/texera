...
class SklearnMultiLayerPerceptronOpDescSpec extends AnyFlatSpec with Matchers {
  ...
  "SklearnMultiLayerPerceptronOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnMultiLayerPerceptronOpDesc
    d.target = "y"
  ...
  "SklearnMultiLayerPerceptronOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnMultiLayerPerceptronOpDesc
    d.target = "label"
