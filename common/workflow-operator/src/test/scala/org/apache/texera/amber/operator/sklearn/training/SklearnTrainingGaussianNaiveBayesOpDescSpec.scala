...
class SklearnTrainingGaussianNaiveBayesOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingGaussianNaiveBayesOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingGaussianNaiveBayesOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Gaussian Naive Bayes"
    info.operatorDescription shouldBe "Sklearn Training: Gaussian Naive Bayes Operator"
  ...
  "SklearnTrainingGaussianNaiveBayesOpDesc" should "default its config fields" in {
  ...
  "SklearnTrainingGaussianNaiveBayesOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingGaussianNaiveBayesOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.naive_bayes import GaussianNB")
    code should include("make_pipeline")
    code should include("Training: Gaussian Naive Bayes")
  }

  "SklearnTrainingGaussianNaiveBayesOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingGaussianNaiveBayesOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingGaussianNaiveBayes\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingGaussianNaiveBayesOpDesc]
