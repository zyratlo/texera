...
class SklearnTrainingRandomForestOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingRandomForestOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingRandomForestOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Random Forest"
    info.operatorDescription shouldBe "Sklearn Training: Random Forest Operator"
  ...
  "SklearnTrainingRandomForestOpDesc" should "default its config fields" in {
    val d = new SklearnTrainingRandomForestOpDesc
  ...
  "SklearnTrainingRandomForestOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingRandomForestOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.ensemble import RandomForestClassifier")
    code should include("make_pipeline")
    code should include("Training: Random Forest")
  }

  "SklearnTrainingRandomForestOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingRandomForestOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingRandomForest\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingRandomForestOpDesc]
    val r = restored.asInstanceOf[SklearnTrainingRandomForestOpDesc]
