...
class SklearnRidgeCVOpDescSpec extends AnyFlatSpec with Matchers {
  ...
  "SklearnRidgeCVOpDesc.operatorInfo" should
    ...
    info.operatorDescription shouldBe "Sklearn Ridge Regression Cross Validation Operator"
  ...
  "SklearnRidgeCVOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnRidgeCVOpDesc
    d.target = "y"
  ...
  "SklearnRidgeCVOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnRidgeCVOpDesc
    d.target = "label"
