...
class SklearnDummyClassifierOpDescSpec extends AnyFlatSpec with Matchers {
  ...
  "SklearnDummyClassifierOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnDummyClassifierOpDesc
    d.target = "y"
  ...
  "SklearnDummyClassifierOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnDummyClassifierOpDesc
    d.target = "label"
