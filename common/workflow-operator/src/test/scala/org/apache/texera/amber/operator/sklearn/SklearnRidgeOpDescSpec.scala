...
class SklearnRidgeOpDescSpec extends AnyFlatSpec with Matchers {
  ...
  "SklearnRidgeOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnRidgeOpDesc
    d.target = "y"
  ...
  "SklearnRidgeOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnRidgeOpDesc
    d.target = "label"
