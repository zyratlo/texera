...
class SklearnTrainingLogisticRegressionOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingLogisticRegressionOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingLogisticRegressionOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Logistic Regression"
    info.operatorDescription shouldBe "Sklearn Training: Logistic Regression Operator"
  ...
  "SklearnTrainingLogisticRegressionOpDesc" should "default its config fields" in {
    val d = new SklearnTrainingLogisticRegressionOpDesc
  ...
  "SklearnTrainingLogisticRegressionOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingLogisticRegressionOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.linear_model import LogisticRegression")
    code should include("make_pipeline")
    code should include("Training: Logistic Regression")
  }

  "SklearnTrainingLogisticRegressionOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingLogisticRegressionOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingLogisticRegression\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingLogisticRegressionOpDesc]
    val r = restored.asInstanceOf[SklearnTrainingLogisticRegressionOpDesc]
