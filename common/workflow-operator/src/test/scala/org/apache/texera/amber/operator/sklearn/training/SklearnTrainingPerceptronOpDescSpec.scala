...
class SklearnTrainingPerceptronOpDescSpec extends AnyFlatSpec with Matchers {

  "SklearnTrainingPerceptronOpDesc.operatorInfo" should
    "advertise the model name, Sklearn Training group, and the single training port" in {
    val info = (new SklearnTrainingPerceptronOpDesc).operatorInfo
    info.userFriendlyName shouldBe "Training: Linear Perceptron"
    info.operatorDescription shouldBe "Sklearn Training: Linear Perceptron Operator"
  ...
  "SklearnTrainingPerceptronOpDesc" should "default its config fields" in {
    val d = new SklearnTrainingPerceptronOpDesc
  ...
  "SklearnTrainingPerceptronOpDesc.generatePythonCode" should "import the configured sklearn estimator" in {
    val d = new SklearnTrainingPerceptronOpDesc
    d.target = "y"
    val code = d.generatePythonCode()
    code should include("from sklearn.linear_model import Perceptron")
    code should include("make_pipeline")
    code should include("Training: Linear Perceptron")
  }

  "SklearnTrainingPerceptronOpDesc" should "round-trip its config fields through the polymorphic base" in {
    val d = new SklearnTrainingPerceptronOpDesc
    d.target = "label"
    d.countVectorizer = true
    val json = objectMapper.writeValueAsString(d)
    json should include("\"operatorType\":\"SklearnTrainingPerceptron\"")
    val restored = objectMapper.readValue(json, classOf[LogicalOp])
    restored shouldBe a[SklearnTrainingPerceptronOpDesc]
    val r = restored.asInstanceOf[SklearnTrainingPerceptronOpDesc]
