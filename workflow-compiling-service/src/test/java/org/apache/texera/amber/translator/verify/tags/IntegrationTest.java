 ...
 * (which passes {@code -l org.apache.texera.amber.translator.verify.tags.IntegrationTest}
 * to ScalaTest), and the integration job uses {@code integration-only} (which
 * passes {@code -n} for the same tag).
 *
 * <p>Mirrors amber's {@code org.apache.texera.amber.tags.IntegrationTest}. Only
 * {@code OperatorBehaviorSpec} carries this tag today — it is the sole spec that
