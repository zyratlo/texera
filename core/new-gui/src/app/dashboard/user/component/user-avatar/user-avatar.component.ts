...
export class UserAvatarComponent implements OnInit {
  ...
  private googleAPI = environment.googleAPI;
  ...
  ngOnInit(): void {
    if (!this.googleId && !this.userName) {
      ...
      const googlePeopleAPIUrl = `https://people.googleapis.com/v1/people/${this.googleId}?personFields=names%2Cphotos&key=${this.googleAPI}`;
      this.http
