...
class HashJoinPrincipalSpec
  ...
  "A HashJoinPrincipal with 5 workers" should "join 2 datasets by the first column" in {
    ...
    sendActor1 ! UpdateOutputLinking(
      new HashBasedShufflePolicy(1, innerHash),
      LinkTag(innertableTag, joinTag),
      joinMetadata.topology.layers.head.layer.map(new DirectRoutee(_))
    )
    sendActor2 ! UpdateOutputLinking(
      new HashBasedShufflePolicy(1, outerHash),
      LinkTag(outertableTag, joinTag),
      joinMetadata.topology.layers.head.layer.map(new DirectRoutee(_))
    )
